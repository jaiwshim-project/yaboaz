(function(){
  ...
  function openModal(){
    ...
    function setMode(next){mode=next;b.querySelectorAll('[data-auth-mode]').forEach(function(x){x.classList.toggle('active',x.dataset.authMode===mode);});nameField.hidden=mode!=='signup';nameField.querySelector('input').required=mode==='signup';submit.textContent=mode==='signup'?'회원가입':'로그인';message.textContent='';form.reset();}
