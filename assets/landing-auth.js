(function(){
  ...
  function openModal(){
    ...
    b.innerHTML='<section class="auth-modal" role="dialog" aria-modal="true" aria-labelledby="auth-title"><button class="auth-close" type="button" aria-label="Close">×</button><div class="auth-brand"><img src="assets/kfde-symbol.jpg" alt="YABOAZ K-FDE symbol"><div><strong>YABOAZ</strong><span>K-FDE PLATFORM</span></div></div><div class="auth-kicker">YABOAZ K-FDE ACCESS</div><h2 id="auth-title">로그인하고 플랫폼 시작하기</h2><p class="auth-intro">13단계 실행 플랫폼을 시작하려면 로그인하거나 회원가입하세요.</p><div class="auth-tabs"><button type="button" class="active" data-auth-mode="login">로그인</button><button type="button" data-auth-mode="signup">회원가입</button></div><form class="auth-form"><label>이메일<input type="email" name="email" autocomplete="email" required placeholder="you@example.com"></label><label>비밀번호<input type="password" name="password" autocomplete="current-password" required minlength="6" placeholder="6자 이상"></label><label class="auth-name" hidden>이름<input type="text" name="name" autocomplete="name" placeholder="이름"></label><label class="auth-company" hidden>소속(회사)명<input type="text" name="company" autocomplete="organization" placeholder="소속 또는 회사명"></label><label class="auth-password-confirm" hidden>비밀번호 확인<input type="password" name="passwordConfirm" autocomplete="new-password" minlength="6" placeholder="비밀번호를 다시 입력하세요"></label><p class="auth-message" role="status"></p><button class="auth-resend" type="button" hidden>인증 이메일 다시 보내기</button><button class="auth-submit" type="submit">로그인</button></form><footer class="auth-footer"><div><strong>YABOAZ K-FDE Platform</strong><span>FDE FIELD OPERATING SYSTEM</span></div><div class="auth-footer-status"><i></i><span>OPERATING NORMALLY</span></div><small>HUMAN IN THE LOOP · EVIDENCE DRIVEN</small></footer></section>';
    document.body.appendChild(b); var mode='login',form=b.querySelector('form'),submit=b.querySelector('.auth-submit'),message=b.querySelector('.auth-message'),nameField=b.querySelector('.auth-name'),companyField=b.querySelector('.auth-company'),passwordConfirmField=b.querySelector('.auth-password-confirm'),resend=b.querySelector('.auth-resend');
    function setMode(next){mode=next;b.querySelector('.auth-modal').classList.toggle('signup-mode',mode==='signup');b.querySelectorAll('[data-auth-mode]').forEach(function(x){x.classList.toggle('active',x.dataset.authMode===mode);});nameField.hidden=mode!=='signup';companyField.hidden=mode!=='signup';passwordConfirmField.hidden=mode!=='signup';nameField.querySelector('input').required=mode==='signup';companyField.querySelector('input').required=mode==='signup';passwordConfirmField.querySelector('input').required=mode==='signup';resend.hidden=true;submit.textContent=mode==='signup'?'회원가입':'로그인';message.textContent='';form.reset();}
    b.querySelectorAll('[data-auth-mode]').forEach(function(x){x.onclick=function(){setMode(x.dataset.authMode);};});
    b.querySelector('.auth-close').onclick=function(){b.remove();}; b.onclick=function(e){if(e.target===b)b.remove();}; resend.onclick=function(){var email=String(new FormData(form).get('email')||'').trim();resend.disabled=true;api('/auth/v1/resend',{method:'POST',body:JSON.stringify({type:'signup',email:email})}).then(function(){message.className='auth-message success';message.textContent='인증 이메일을 다시 보냈습니다. 받은 편지함을 확인해 주세요.';resend.disabled=false;}).catch(function(err){message.className='auth-message error';message.textContent=err.message;resend.disabled=false;});};
    form.onsubmit=function(e){e.preventDefault();var data=new FormData(form),email=String(data.get('email')).trim(),password=String(data.get('password')),passwordConfirm=String(data.get('passwordConfirm')||'');if(mode==='signup'&&password!==passwordConfirm){message.className='auth-message error';message.textContent='비밀번호가 일치하지 않습니다.';return;}submit.disabled=true;message.className='auth-message';message.textContent=mode==='signup'?'회원가입 처리 중…':'로그인 확인 중…';var request=mode==='signup'?api('/auth/v1/signup?redirect_to='+encodeURIComponent(window.location.origin+window.location.pathname),{method:'POST',body:JSON.stringify({email:email,password:password,data:{full_name:String(data.get('name')||'').trim(),company_name:String(data.get('company')||'').trim()}})}):api('/auth/v1/token?grant_type=password',{method:'POST',body:JSON.stringify({email:email,password:password})});request.then(function(result){if(mode==='signup'&&!result.access_token){message.className='auth-message success';message.textContent='가입이 완료되었습니다. 이메일로 전송된 인증 링크를 확인해 주세요.';resend.hidden=false;submit.disabled=false;return;}if(result.access_token){try{sessionStorage.setItem('kfde-auth-session',JSON.stringify(result));}catch(err){}window.location.href=DEST;}}).catch(function(err){message.className='auth-message error';message.textContent=err.message;submit.disabled=false;});};
  }
