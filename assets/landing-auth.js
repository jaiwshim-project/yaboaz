(function(){
  ...
  function openModal(){
    ...
    document.body.appendChild(b); var mode='login',form=b.querySelector('form'),submit=b.querySelector('.auth-submit'),message=b.querySelector('.auth-message'),nameField=b.querySelector('.auth-name'),companyField=b.querySelector('.auth-company');
    function setMode(next){mode=next;b.querySelectorAll('[data-auth-mode]').forEach(function(x){x.classList.toggle('active',x.dataset.authMode===mode);});nameField.hidden=mode!=='signup';companyField.hidden=mode!=='signup';nameField.querySelector('input').required=mode==='signup';companyField.querySelector('input').required=mode==='signup';submit.textContent=mode==='signup'?'회원가입':'로그인';message.textContent='';form.reset();}
