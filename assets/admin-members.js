(function(){
  ...
  function render(rows){var counts={all:rows.length,pending:0,approved:0,blocked:0};rows.forEach(function(x){if(x.status==='pending')counts.pending++;if(x.status==='approved')counts.approved++;if(x.status==='blocked'||x.status==='rejected')counts.blocked++;});Object.keys(counts).forEach(function(k){var el=document.getElementById('count-'+k);if(el)el.textContent=counts[k];});var list=document.getElementById('member-list');if(!list)return;list.innerHTML=rows.length?rows.map(function(x){return '<tr><td><span class="member-name">'+esc(x.full_name)+'</span><span class="member-email">'+esc(x.email)+'</span></td><td>'+esc(x.company_name)+'</td><td>'+esc(x.phone)+'</td><td>'+esc(String(x.created_at||'').slice(0,10))+'</td><td><span class="status status-'+esc(x.status)+'">'+esc({pending:'가입',approved:'정상',rejected:'거절',blocked:'차단'}[x.status]||x.status)+'</span></td><td><button class="member-delete" data-delete-id="'+esc(x.user_id)+'">회원 삭제</button></td></tr>';}).join(''):'<tr><td colspan="6" class="empty">가입 회원이 없습니다.</td></tr>'}
